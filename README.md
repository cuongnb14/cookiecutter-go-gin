# cookiecutter-go-gin

Powered by Cookiecutter. Cookiecutter go gin is a framework for jumpstarting projects go-gin quickly. It use repository + service pattern, very useful for **small** and **medium** projects, for large projects **hexagon architecture** can be used.

## Usage

```sh
pip install cookiecutter
cookiecutter https://github.com/cuongnb14/cookiecutter-go-gin.git
```

## Integrate
- Go fx dependency injection
- Repository + Service pattern
- Asynq - distributed tasks queue
- Asynqmon - dashboard for asynq
- pprof - for debugging
- Gorm - ORM
- Atlas - databse migration
- Slog + Sentry - Logging
- Viper - load config from env
- JWT auth
- Utils command (use invoke python): generate model, repository, service files, format env ...
- And more ...
